                ...
                <c:if test="${success}">
                    <form id="themeForm">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="${theme.title}"
                                   datatype="*" nullmsg="请填写标题" maxlength="50">
                            <input type="hidden" name="guid" value="${theme.guid}">
                            <input type="hidden" name="fid" value="${forum.fid}">
                        </div>
                        <div class="form-group">
                            <label for="title">发布时间</label>
                            <input type="text" class="form-control time-picker" id="publishDate" name="publishDate"
                                   value="<fmt:formatDate value='${theme.publishDate}' pattern='yyyy-MM-dd HH:mm:ss' />"
                                   placeholder="提供可选择的发布日期">
                        </div>
                        <div class="form-group">
                            <label for="active">主题类型</label>
                            <br>
                            <input type="radio" id="typeNormal" name="type"
                                   value="0" ${theme==null?"checked":(theme.type==0?"checked":"")}><label
                                for="typeNormal" style="font-weight: normal;">图文主题</label>
                            <input type="radio" id="typeAudio" name="type"
                                   value="1" ${theme.type==1?"checked":""}><label for="typeAudio"
                                                                                  style="font-weight: normal;">音频主题</label>
                            <input type="radio" id="typeVideo" name="type"
                                   value="2" ${theme.type==2?"checked":""}><label for="typeVideo"
                                                                                  style="font-weight: normal;">视频主题</label>
                        </div>
                        <div class="medias row">
                            <c:if test="${theme.type==2}">
                                <c:forEach items="${theme.medias}" var="media">
                                    <div class="col-md-6">
                                        <input class="form-control url pull-left" type="text" name="files"
                                               value="${media.url}"/>
                                    </div>
                                </c:forEach>
                            </c:if>
                            <c:if test="${theme.type==1}">
                                <c:forEach items="${theme.medias}" var="media">
                                    <div class="col-lg-2">
                                        <input class='form-control atitle pull-left' type='text' name='titles'
                                               placeholder='文件名' value="${media.title}" datatype="*" nullmsg="请填写文件名"/>
                                    </div>
                                    <div class="col-lg-4">
                                        <input class='form-control url pull-left' type='text' name='files'
                                               placeholder='文件地址' value='${media.url}' datatype="*" nullmsg="请填写文件地址"/>
                                    </div>
                                    <div class="col-lg-2">
                                        <input class='form-control singer pull-left' type='text' name='singers'
                                               placeholder='歌手' value="${media.singer}" datatype="*" nullmsg="请填写歌手"/>
                                    </div>
                                    <div class="col-lg-3">
                                        <input class='form-control time pull-left' type='text' name='times'
                                               placeholder='播放时间' value="${media.lastTime}" datatype="*"
                                               nullmsg="请填写播放时长"/>
                                    </div>
                                    <div class="col-lg-1">
                                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="tags" name="tags" value="${theme.tags}"
                                   maxlength="50">
                        </div>
                        ...
                        <div class="btns pull-right">
                            <button type="button" class="btn btn-primary"
                                    onclick="submitTheme('2',this)"
                                    data-loading-text="提交中...">${theme.state!=2?"发布":"保存"}</button>
                            <c:if test="${theme.state!=2}">
                                <button type="button" class="btn btn-success"
                                        onclick="submitTheme('1',this)" data-loading-text="提交中...">草稿
                                </button>
                            </c:if>
                            <button type="button" class="btn btn-default"
                                    onclick="location.href='admin/theme/themeList?fid=${theme.forum.fid}'">返回
                            </button>
                        </div>
                    </form>
                </c:if>
            ...
            .ready(function (K) {
                var editor = K .create(  "#contentEditer", {
                            ...
                            htmlTags: {
                                ...
                                div: ['align', '.border',  '.margin', '.padding', '.text-align',  '.color',  '.background-color', '.font-size', '.font-family',  '.font-weight', '.background', '.font-style',  '.text-decoration', '.vertical-align',  '.margin-left',  'id', 'class', 'data-date', 'data-date-format', 'style'],
                                table: ['border',  'cellspacing',  'cellpadding', 'width',  'height', 'align',  'bordercolor',  '.padding', '.margin',  '.border', 'bgcolor',   '.text-align',  '.color',  '.background-color',  '.font-size', '.font-family',  '.font-weight',  '.font-style',  '.text-decoration',   '.background',  '.width', '.height',   '.border-collapse'], 'td,th': ['align', 'valign',  'width', 'height',  'colspan', 'rowspan',  'bgcolor',  '.text-align',  '.color',  '.background-color',  '.font-size',   '.font-family',   '.font-weight',  '.font-style',  '.text-decoration',  '.vertical-align',   '.background',  '.border'],
                                ...
                                img: ['src', 'width',
                                    ...
                                    'alt', 'title',
                                    'align', '.width',
                                    '.height', '.border'],  'p,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6': [  'align', '.text-align',  '.color',  '.background-color',   '.font-size',   '.font-family',  '.background',   '.font-weight',   '.font-style', '.text-decoration',   '.vertical-align',  '.text-indent',  '.margin-left'],   'pre,span': ['class'],
                                i: ['class'],
